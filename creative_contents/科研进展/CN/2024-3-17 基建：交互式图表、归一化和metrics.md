<h2 style='pointer-events: none;'>基建：交互式图表、归一化和metrics</h2>

基建是一个系列文档，主要是把深度学习进行降雨径流模拟中常见的功能实现写成一个包以实现代码复用。

由于在写代码时发现有些功能每次都要重新写一遍，例如神经网络的loss、绘制降水径流过程图、数据归一化等，这些代码不难但很耗费时间，于是准备花一些时间把这些写成一个叫做***utils***的包
<h3 style='pointer-events: none;'>归一化</h3>
...
```
<h3 style='pointer-events: none;'>交互式图表</h3>
